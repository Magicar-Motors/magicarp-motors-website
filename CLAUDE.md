...
A [Quartz 4](https://quartz.jzhao.xyz/) static site for the Magicarp Motors racing team. Content lives in `content/` as Markdown files; the Quartz framework (in `quartz/`) handles build, theming, and plugins. The deployed site is a knowledge-base / race-log for the team.
